...
net = conv_net(5, 1, [64,128,64], activation='ReLU', in_spatial=2)
optimizer = adam(net)
math.print(parameter_count(net))
net2 = conv_net(5, 1, [64,128,64], activation='ReLU', in_spatial=2)
optimizer2 = adam(net2)
math.print(parameter_count(net2))
net3 = conv_net(5, 1, [64,128,64], activation='ReLU', in_spatial=2)
optimizer3 = adam(net3)
net4 = conv_net(5, 1, [64,128,64], activation='ReLU', in_spatial=2)
optimizer4 = adam(net4)
net5 = conv_net(5, 1, [64,128,64], activation='ReLU', in_spatial=2)
...
for steps in range(num_train_steps):
    ...
    loss4, _, _ = update_weights(net4, optimizer4, training_loss_noise_2, input_nn_conv_noise_2)
    ...
    if steps % k == 0:
        ...
        print(f"Predicted_NN: {math.mean(math.native_call(net, input_nn_conv)), math.mean(math.native_call(net2, input_nn_conv_noise_1)), math.mean(math.native_call(net3, input_nn_conv_noise_1)), math.mean(math.native_call(net4, input_nn_conv_noise_2)), math.mean(math.native_call(net5, input_nn_conv_noise_2))}")
        print(f"Training_loss_NN: {loss, loss2, loss3, loss4, loss5}")
        save_state(net, './billiards_ConvNet_3.1_28032023_01.pth')
        save_state(net2, './billiards_ConvNet_3.1_28032023_noise_1_01.pth')
        save_state(net3, './billiards_ConvNet_3.1_28032023_noise_1_rf_01.pth')
        save_state(net4, './billiards_ConvNet_3.1_28032023_noise_2_01.pth')
        save_state(net5, './billiards_ConvNet_3.1_28032023_noise_2_rf_01.pth')
...
alpha_test = math.random_uniform(batch(b=2), low=parm_1_min, high=parm_1_max)
...
def landscape(alpha):
  ...
  input_nn = math.rename_dims(trj_test.elements.center, instance('balls'), spatial('balls'))
...
show({"Ground Truth": CenteredGrid(lambda alpha: landscape(alpha)[0], alpha=500, bounds=Box(alpha=(parm_1_min,parm_1_max)))})
show({"ConvNET predicted": CenteredGrid(lambda alpha: landscape(alpha)[1], alpha=500, bounds=Box(alpha=(parm_1_min,parm_1_max)))})
show({"ConvNET Noise 1": CenteredGrid(lambda alpha: landscape(alpha)[2], alpha=500, bounds=Box(alpha=(parm_1_min,parm_1_max)))})
show({"ConvNET Noise1 rf": CenteredGrid(lambda alpha: landscape(alpha)[3], alpha=500, bounds=Box(alpha=(parm_1_min,parm_1_max)))})
show({"ConvNET Noise 2": CenteredGrid(lambda alpha: landscape(alpha)[4], alpha=500, bounds=Box(alpha=(parm_1_min,parm_1_max)))})
show({"ConvNEt Noise 2 rf": CenteredGrid(lambda alpha: landscape(alpha)[5], alpha=500, bounds=Box(alpha=(parm_1_min,parm_1_max)))})


vis.show(CenteredGrid(lambda alpha: math.stack({"Ground Truth": landscape(alpha)[0],"ConvNet predicted": landscape(alpha)[1], "ConvNet noise1": landscape(alpha)[2], "ConvNet noise1_rf": landscape(alpha)[3]}, channel('curves')), alpha=1000, bounds=Box(alpha=(parm_1_min,parm_1_max))))
vis.show(CenteredGrid(lambda alpha: math.stack({"Ground Truth": landscape(alpha)[0],"ConvNet predicted": landscape(alpha)[1], "ConvNet noise2": landscape(alpha)[4], "ConvNet noise2_rf": landscape(alpha)[5]}, channel('curves')), alpha=1000, bounds=Box(alpha=(parm_1_min,parm_1_max))))
vis.show(CenteredGrid(lambda alpha: math.stack({"Ground Truth": landscape(alpha)[0],"ConvNet noise_1_rf": landscape(alpha)[3], "ConvNet noise2_rf": landscape(alpha)[5]}, channel('curves')), alpha=1000, bounds=Box(alpha=(parm_1_min,parm_1_max))))
vis.show(CenteredGrid(lambda alpha: math.stack({"Ground Truth": landscape(alpha)[0],"ConvNet predicted": landscape(alpha)[1], "ConvNet noise1": landscape(alpha)[2], "ConvNet noise1_rf": landscape(alpha)[3], "ConvNet noise2": landscape(alpha)[4], "ConvNet noise2_rf": landscape(alpha)[5]}, channel('curves')), alpha=1000, bounds=Box(alpha=(parm_1_min,parm_1_max))))
